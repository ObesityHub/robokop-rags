...
def test_gwas_finding_significant_hits():
    ...
    with GWASFileReader(GWASFile('./sample_data/sample_sugen.gz')) as test_file_reader:
    ...
    with GWASFileReader(GWASFile('./sample_data/fake_file_name.gz')) as test_file_reader:
...
def test_mwas_finding_significant_hits():
    with MWASFileReader(MWASFile('./sample_data/sample_mwas')) as test_file_reader:
        found_variants, variant_container, num_found = test_file_reader.find_significant_hits(0.005)
        assert found_variants
        assert num_found == 5
...
def test_get_gwas_association_from_file():
    ...
    with GWASFileReader(GWASFile('./sample_data/sample_sugen2.gz'), use_tabix=True) as test_file_reader_1:
    ...
    with GWASFileReader(GWASFile('./sample_data/sample_sugen2.gz')) as test_file_reader_2:
