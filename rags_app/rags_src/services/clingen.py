...
logger = LoggingUtil.init_logging("rags.service_clingen", logging.INFO, format='medium', logFilePath=f'{os.environ["RAGS_HOME"]}/logs/')
                ...
                for genomic_allele in allele_json['genomicAlleles']:
                    ...
                    if 'referenceGenome' in genomic_allele and genomic_allele['referenceGenome'] == 'GRCh38':
                        ...
                        if 'chromosome' in genomic_allele:
                            ...
                            synonyms.add(
                                LabeledID(identifier=f'ROBO_VARIANT:{robokop_variant_id}', label=robokop_variant_id))
                    ...
                    for allele_json in query_results:
                        if '@id' in allele_json:
                            ...
                            if (len(id_split) > 1) and ('CA' in id_split[1]):
                                ...
                                predicate = LabeledID(identifier=f'clingen.gene_to_sequence_variant',
                                                      label=f'gene_to_sequence_variant')
                                edge = self.create_edge(gene_node, variant_node, 'clingen.gene_to_sequence_variant',
                                                        currie_gene_symbol, predicate, url=query_url)
